...
/*        //获取图片上传
        ...
        if (level1Id != 0 && level2Id != 0 && level3Id != 0) {//添加最后一级商品
            ...
            if (pDao.add(ep) > 0) {
                printWriter.write("<script>alert('添加成功');location.href='/ning.ying/MemberServlet?method=memberProductListLeft';</script>");
            } else {
                printWriter.write("<script>alert('添加失败');location.href='/ning.ying/MemberServlet?method=memberProductListLeft';</script>");
            }
        }
    }
