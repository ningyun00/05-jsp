...
<html xmlns = "http://www.w3.org/1999/xhtml">
            ...
            <div class = "m_content">
                ...
                <div class = "m_right">
                    ...
                    <table border = "0" class = "order_tab" style = "width:930px; text-align:center; margin-bottom:30px;" cellspacing = "0" cellpadding = "0">
                        ...
                        <c:forEach items = "${sessionScope.productCategoryList}" var = "productCategoryList" varStatus = "i">
                            <tr>
                                ...
                                <td>
                                    <c:if test = "${productCategoryList.type==1}">无</c:if>
                                    <c:forEach items = "${sessionScope.CategoryList}" var = "p">
                                        <c:if test = "${productCategoryList.type==2 && p.id==productCategoryList.parentId}">${p.name}</c:if>
                                        <c:if test = "${productCategoryList.type==3 && p.id==productCategoryList.parentId}">${p.name}</c:if>
                                    </c:forEach>
                                </td>
                                <td width = "145"><a onclick = "CUpdate(${productCategoryList.id})">修改</a></td>
                                <td width = "145"><a onclick = "return alert('无法删除商品分类')">删除</a></td>
                            </tr>
                        </c:forEach>
                    </table>
    ...
    <div class = "update_div1" style = "display: none">
        <div class = "update_div2">
            <form action = "${pageContext.request.contextPath}/memberProductCategoryListServlet?method=update" method = "post">
                <h2 style = "color: red;text-align: center; line-height: 50px">修改分类名称</h2>
                <table style = "margin: auto;padding: 0px;collapse: 0px;">
                    <thead style = "margin: 0px;padding: 0px">
                        <input type = "hidden" id = "userId" name = "id">
                        <tr>
                            <td>原名称</td>
                            <td><input type = "text" readonly value = "" id = "CName1" name = "userName1"></td>
                        </tr>
                        <tr>
                            <td>现名称</td>
                            <td><input type = "text" value = "" id = "CName2" name = "userName2"></td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
    ...
    <script type = "text/javascript" src = "../js/jquery-1.12.4.js"></script>
    <script type = "text/javascript">
        function CUpdate(CID) {
            $("#update").html($(".update_div1").html());
            $("#userId").val(CID);
            $.post(
                "${pageContext.request.contextPath}/memberProductCategoryListServlet?method=updateC",
                {CID: CID},
                function (msg){
                    $("#CName1").val(msg);
                }
