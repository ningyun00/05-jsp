...
public class ProductDao {
    ...
    public List<EabyNews> news() {
        ...
        try {
            connection = BaseDao.getConnection();
            for (int i = 1; i <= 4; i++) {
                preparedStatement = connection.prepareStatement("SELECT * FROM eaby_news LIMIT ?,5");
                preparedStatement.setInt(1,new Random().nextInt(17));
                resultSet = preparedStatement.executeQuery();
                while (resultSet.next()) {
    ...
    public EabyProduct queryById(int id) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                pt.setIsHot(resultSet.getInt("isHot"));
                pt.setIsSale(resultSet.getInt("isSale"));
                pt.setDetailed(resultSet.getString("detailed"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public List<EabyProduct> selectAll(int startLimit, int endLimit) {
        ...
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT * FROM eaby_product WHERE isDelete = 0 ORDER BY id DESC LIMIT ?,?");
    ...
    public int maxRow() {
        ...
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT COUNT(*) FROM `eaby_product` WHERE isDelete = 0");
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
     ...
     * 所有商品分类最大行数
     */
    public int maxRowCategory() {
        int count = 0;
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT COUNT(*) FROM `eaby_product_category`");
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                count = resultSet.getInt(1);
            }
     ...
     * 所有分类
     */
    public List<EabyProductCategory> getCategoryList(int startLimit, int endLimit) {
        List<EabyProductCategory> categoryList = new ArrayList<>();
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT * FROM `eaby_product_category` ORDER BY TYPE ASC LIMIT ?,?");
            preparedStatement.setInt(1, (startLimit - 1) * endLimit);
            preparedStatement.setInt(2, endLimit);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                ...
                eabyProductCategory.setParentId(resultSet.getInt("parentId"));
                eabyProductCategory.setType(resultSet.getInt("type"));
                categoryList.add(eabyProductCategory);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public List<EabyProductCategory> getCategoryList() {
        ...
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT * FROM `eaby_product_category` ORDER BY TYPE ASC");
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                EabyProductCategory eabyProductCategory = new EabyProductCategory();
                eabyProductCategory.setId(resultSet.getInt("id"));
     ...
     * 根据编号查询分类
     */
    public EabyProductCategory selectCID(int CID) {
        EabyProductCategory eabyProductCategory = new EabyProductCategory();
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT * FROM `eaby_product_category` WHERE id = ?");
        ...
        return eabyProductCategory;
    }

    /**
     * 查询商品父类
     */
    public List<EabyProductCategory> categoryTYPE(int PID) {
        List<EabyProductCategory> list = new ArrayList<>();
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("SELECT * FROM `eaby_product_category` WHERE parentId = ?");
     ...
     * 修改商品分类名称
     */
    public int categoryUpdate(EabyProductCategory eabyProductCategory) {
        int row = 0;
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("UPDATE  `eaby_product_category` SET NAME = ? WHERE id = ?");
            preparedStatement.setString(1, eabyProductCategory.getName());
            preparedStatement.setInt(2, eabyProductCategory.getId());
            row = preparedStatement.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public int delete(int id) {
        ...
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("UPDATE `eaby_product` SET isDelete=1 WHERE id = ?");
            preparedStatement.setInt(1, id);
            row = preparedStatement.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * 修改商品
     */
    public int update(EabyProduct product) {//修改商品
        int row = 0;
        try {
            connection = BaseDao.getConnection();
            String sql = "UPDATE `eaby_product` SET `eaby_product`.`name` = ?,description = ?,price = ?,stock = ?,isDelete = ?,isHot = ?,isSale = ?,detailed = ? WHERE id = ?";
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, product.getName());
            preparedStatement.setString(2, product.getDescription());
            preparedStatement.setDouble(3, product.getPrice());
            preparedStatement.setInt(4, product.getStock());
            preparedStatement.setInt(5, product.getIsDelete());
            preparedStatement.setInt(6, product.getIsHot());
            preparedStatement.setInt(7, product.getIsSale());
            preparedStatement.setString(8, product.getDetailed());
