    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("loginout".equals(method)) {
            ...
            for (Cookie c : cookies) {
                if ("causer".equals(c.getName())&&c.getName()!=null) {
                    if (!"".equals(c.getName())) {
                        Cookie cookie = new Cookie("causer", null);
                        cookie.setMaxAge(60 * 60 * 24 * 3);
                        resp.addCookie(cookie);
                    }
                } else if ("causer0".equals(c.getName())) {
                    Cookie cookie = new Cookie("causer0", null);
                    cookie.setMaxAge(60 * 60 * 24 * 3);
                    resp.addCookie(cookie);
                }
            }
        ...
        if (eabyUser == null) {
            ...
            if (type == 1) {
                if (req.getParameter("checkName") != null && !"".equals(req.getParameter("checkName"))) {
                    Cookie cookie = new Cookie("causer", URLEncoder.encode(loginName + "," + loginPassword + "," + eabyUser.getId(), "utf-8"));
                    cookie.setMaxAge(60 * 60 * 24 * 3);
                    resp.addCookie(cookie);
                    //测试cookie
                ...
                Cookie cookie = new Cookie("causer0", URLEncoder.encode(loginName + "," + loginPassword + "," + eabyUser.getId(), "utf-8"));
                cookie.setMaxAge(60 * 60 * 24 * 3);
                resp.addCookie(cookie);
                //测试cookie
                System.out.println("UserServlet.java ---login  打印cookie：" + cookie.getName());
