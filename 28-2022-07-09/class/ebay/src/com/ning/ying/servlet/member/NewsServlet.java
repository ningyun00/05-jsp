...
public class NewsServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("selectNewsS".equals(method)) {
            selectNewsS(request, response);
        }
    }
     ...
     * 查询新闻是否重复
     */
    private void selectNewsS(HttpServletRequest req, HttpServletResponse resp) {
        String name = req.getParameter("titleNameS");
        System.out.println(newsDao.TitleNews(name).getTitle());
        if (name.equals(newsDao.TitleNews(name).getTitle())){
            printWriter.write("标题名重复");
        }else{
            printWriter.write("");
        }
    }
    ...
    private void addNews(HttpServletRequest req, HttpServletResponse resp) {
        ...
        String contentCard = req.getParameter("contentCard");
        EabyNews eabyNews = new EabyNews();
        eabyNews.setTitle(titleName);
        eabyNews.setContent(contentCard);
        if (newsDao.addNews(eabyNews) > 0) {
            printWriter.write("<script>alert('添加成功');location.href='/ning.ying/MemberServlet?method=memberNews';</script>");
        } else {
            printWriter.write("<script>alert('添加失败');location.href='/ning.ying/MemberServlet?method=memberNews';</script>");
        }
    }
    ...
    private void deleteNews(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        int newId = Integer.parseInt(String.valueOf(req.getParameter("newsId")));
