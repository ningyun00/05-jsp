...
public class UserDao {
     ...
     * 修改用户信息
     */
    public int updateUserAddress(EabyUser eabyUser) {
        int row = 0;
        try {
            connection = BaseDao.getConnection();
            preparedStatement = connection.prepareStatement("UPDATE `eaby_user` SET userName = ?,email = ?,mobile = ? WHERE id = ?");
            preparedStatement.setString(1,eabyUser.getUserName());
            preparedStatement.setString(2,eabyUser.getEmail());
            preparedStatement.setString(3, eabyUser.getMobile());
            preparedStatement.setInt(4, eabyUser.getId());
            row = preparedStatement.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
